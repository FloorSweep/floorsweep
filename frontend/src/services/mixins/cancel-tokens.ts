...
export function CancelableTokens<T extends Constructor>(Base1: T) {
    ...
    const CancelableTokens = class extends Base1 {
        ...
        protected _cancelable_tokens_construct(cancelIdPrefix: string, cancelId: any = Math.random()) {
        ...
        private guardConstructor() {
            if (!this._has_called_constructor) {
                throw new Error("CancelableTokens has been used before calling constructor");
            }
        }
        ...
        public generate(funcName: any, ...args: any) {
            ...
            let key: any = ((/^function\s+([\w]+)\s*\(/.exec(funcName.toString())) as any);
